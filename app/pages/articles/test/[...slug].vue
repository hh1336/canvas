...
const path = computed(() => withLeadingSlash(joinURL(locale.value, 'articles', 'test', ...slug.value)))
const collection = computed(() => `articles_${locale.value}_test` as keyof Collections)

const { data: page } = await useAsyncData(path.value, async () =>
  await queryCollection(collection.value).path(path.value).first() as Collections['articles_en_test'],
)
console.log('执行了', page.value)
